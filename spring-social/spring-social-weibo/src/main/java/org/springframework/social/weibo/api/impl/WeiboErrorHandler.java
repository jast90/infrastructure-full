...
public class WeiboErrorHandler extends DefaultResponseErrorHandler {
     ...
     *
     * @param response response from Weibo server
     * @throws IOException
     */
    ...
    private void handleServerErrors(HttpStatus statusCode) throws IOException {
        if (statusCode == HttpStatus.INTERNAL_SERVER_ERROR) {
            throw new InternalServerErrorException("weibo", "Something is broken at Weibo.");
        } else if (statusCode == HttpStatus.BAD_GATEWAY) {
            throw new ServerDownException("weibo", "Weibo is down or is being upgraded.");
        } else if (statusCode == HttpStatus.SERVICE_UNAVAILABLE) {
            throw new ServerOverloadedException("weibo", "Weibo is overloaded with requests. Try again later.");
        }
    }
