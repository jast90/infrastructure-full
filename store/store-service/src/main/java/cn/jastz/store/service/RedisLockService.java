...
/**
 * @author zhangzhiwen on 2019/11/5
 **/
@Service
public class RedisLockService {
     ...
     * 重试时间
     */
    private static final long retryTime = 1;
     ...
     * @return
     */
    public boolean lock(String lock, long timeout, TimeUnit timeUnit) {
        long startTime = System.currentTimeMillis();
        if (getLock(lock, timeout, timeUnit)) {
            return true;
        }
        /*long times = timeUnit.toMillis(timeout);
        while (true) {
            boolean isTimeout = System.currentTimeMillis() - startTime - times >= 0;
            if (isTimeout) {
                break;
            }
            if (getLock(lock, timeout, timeUnit)) {
    ...
    public boolean getLock(String lock, long timeout, TimeUnit timeUnit) {
        return redisTemplate.opsForValue().setIfAbsent(getKey(lock), getKey(lock),
                timeout, timeUnit);
    }

    private String getKey(String lock) {
        return String.format("redis:lock:%s", lock);
    }

    public boolean unlock(String lock) {
        if (redisTemplate.hasKey(getKey(lock))) {
            return redisTemplate.delete(getKey(lock));
        }
        return false;
    }
